...
def _import_unidiff_patchset():
    """Import and return unidiff.PatchSet, attempting on-the-fly install if missing.

    This makes the Python fallback more reliable in environments where
    the optional dependency isn't preinstalled.
    """
    try:
        from unidiff import PatchSet  # type: ignore
        return PatchSet
    except Exception as first_error:  # ImportError or others
        # Try a best-effort non-interactive installation of unidiff
        try:
            import sys
            import subprocess as _sub
            _sub.run(
                [sys.executable, "-m", "pip", "install", "unidiff>=0.7.5", "--quiet"],
                check=True,
                capture_output=True,
                text=True,
            )
            ...
            raise RuntimeError(
                "unidiff package is required to apply patches and could not be installed automatically"
            ) from second_error
...
def apply_unified_diff(repo_root: Path, diff_text: str) -> List[Path]:
    ...
    """
    # Import (or install) unidiff on demand for Python-based fallback
    PatchSet = _import_unidiff_patchset()
    ...
    try:
        ...
        if not changed_files:
            if verbose:
                ...
                print("Attempting Python-based fallback patch application...")
            try:
                fallback_changed = apply_unified_diff(repo_root, diff_text)
                if fallback_changed:
                    if verbose:
                        print("Fallback succeeded; files changed:", ", ".join(str(p) for p in fallback_changed))
