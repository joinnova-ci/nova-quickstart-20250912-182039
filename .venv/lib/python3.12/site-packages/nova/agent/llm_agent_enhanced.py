...
class EnhancedLLMAgent:
    ...
    def __init__(self, repo_path: Path, verbose: bool = False, truncated: bool = False):
        self.repo_path = repo_path
        self.settings = get_settings()
        self.llm = LLMClient(verbose=verbose, truncated=truncated)  # Use the unified LLM client
        self.verbose = verbose
        self.truncated = truncated
        self.ignore_patterns: List[str] = self._load_ignore_rules()

    def _load_ignore_rules(self) -> List[str]:
        """Load ignore patterns from defaults, env, and optional .novaignore file."""
        # Sensible defaults to avoid pulling in virtualenvs and build artifacts
        patterns: List[str] = [
            ".venv",
            "venv",
            "env",
        ...
        # NOVA_IGNORE_PATHS can be CSV or newline-separated list of glob patterns or dir names
        env_val = os.environ.get("NOVA_IGNORE_PATHS")
        if env_val:
            extra = []
            for chunk in env_val.replace("\n", ",").split(","):
                token = chunk.strip()
                if token:
                    extra.append(token)
            patterns.extend(extra)

        # Optional .novaignore file in repo root
        try:
            ignore_file = self.repo_path / ".novaignore"
            ...
            # Ignore errors reading .novaignore
            pass
    ...
    def _is_ignored_path(self, path: Path) -> bool:
        """Return True if path should be ignored based on configured patterns."""
        try:
            rel = path.relative_to(self.repo_path)
        except Exception:
            # Anything outside the repo root is ignored
            return True
        ...
        for pattern in self.ignore_patterns:
            ...
            if any(c in pattern for c in "*?["):
                if fnmatch.fnmatch(rel_str, pattern):
                    return True
                # Also try to anchor within any subpath
                if fnmatch.fnmatch("/" + rel_str, "*/" + pattern):
                    return True
            else:
                ...
                if token in rel_parts:
                    return True
                # Fallback: substring match for tokens like "site-packages"
    ...
    def find_source_files_from_test(self, test_file_path: Path) -> Set[str]:
        ...
        stdlib_like = {
            ...
            # Common third‑party libs we never want to patch
            ...
            for pf in candidates:
                try:
                    if pf.exists() and self.repo_path in pf.parents:
                        if self._is_ignored_path(pf):
                            # Skip virtualenvs, caches, build artifacts, etc.
                            continue
    ...
    def review_patch(self, patch: str, failing_tests: List[Dict[str, Any]], state=None, test_results_after_patch: Optional[str] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            if response:
                if self.verbose:
                    print(f"[Nova Debug - Critic] Full response:\n{'='*80}\n{response}\n{'='*80}")
            else:
                if self.verbose:
    ...
    def create_plan(self, failing_tests: List[Dict[str, Any]], iteration: int, critic_feedback: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.verbose:
                print(f"[Nova Debug] Plan response from LLM (full):\n{'='*80}\n{response}\n{'='*80}")
